...
const LoginPage = () => {
  ...
  const requestAuthorization = () => {
    let url = `https://accounts.spotify.com/authorize?client_id=${clientId}&response_type=code&redirect_uri=${redirectUri}&show_dialog=true&scope=user-top-read playlist-read-collaborative playlist-modify-public playlist-read-private playlist-modify-private`;
  ...
  return (
    <StylesProvider injectFirst>
      <StyledWrapper onLoad={onPageLoad()}>
        <WelcomeTitle variant="h1">Spotify Stories</WelcomeTitle>
        <DescriptionSubtitle>
          Learn more about your history on spotify.
        </DescriptionSubtitle>
        <GreenButton onClick={() => requestAuthorization()}>
          Login With Spotify
        </GreenButton>
      </StyledWrapper>
    </StylesProvider>
